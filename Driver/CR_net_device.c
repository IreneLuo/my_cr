...
static unsigned long pop_skb_from_buffer(void* dst, struct myRXBuf *buf)
{
	...
	{
		/* [Tracing Note]
		 * Well I don't know how to describe my feeling right now. ;(
		 * The following code try to get two bytes and cascading them into an unsinged short,
		 * The last part forgot to add the 0xff mask like packet_length_if_ready() function,
		 * which I believe may cause some problem.
		 */
